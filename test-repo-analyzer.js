...
async function analyzeRepository(repoUrl, userId = 1) {
  ...
  log(`\n💡 Max Turns: 100 (unlimited exploration mode)`, 'yellow');
  log(`   Note: Large repos may cost $0.10-$0.50 in API usage`, 'yellow');
...
`;
    ...
    const result = await claudeAgent.executeTaskWithGitHub(
      ...
      prompt,
      {
        maxTurns: 100, // Very high limit - allows Claude to freely explore complex repos
    ...
    // Check if Claude hit the turn limit
    const maxTurnsAllowed = 100;
    if (result.metadata.num_turns >= maxTurnsAllowed) {
      log(`\n⚠️  Warning: Claude used all ${maxTurnsAllowed} turns (hit the limit!)`, 'yellow');
      log(`   This repository is extremely large/complex!`, 'yellow');
    } else if (result.metadata.num_turns >= 50) {
      log(`\n⚠️  Note: Claude used ${result.metadata.num_turns} turns (repository is very complex)`, 'yellow');
    }
    ...
    if (overviewSaved && overviewContent) {
      ...
      log(`\n⚠️  PROJECT_OVERVIEW.md was NOT created!`, 'red');

      if (result.metadata.num_turns >= maxTurnsAllowed) {
        log(`   Reason: Claude hit the ${maxTurnsAllowed} turn limit before finishing.`, 'yellow');
        log(`   This repository is too large/complex for the current turn limit.`, 'yellow');
        log(`   \n   Suggestions:`, 'cyan');
        log(`   - Try analyzing a smaller/simpler repository`, 'cyan');
        log(`   - Or increase maxTurns in the script (but this will cost more)`, 'cyan');
      } else {
        log(`   Claude completed ${result.metadata.num_turns} turns but didn't create the file.`, 'yellow');
        log(`   This is unexpected - Claude may have encountered an error.`, 'yellow');
      }
      log('', 'reset');
    }
